...
spiral_custom.draw = function() {
  ...
  let time = 0.0;
    ...
    for(var i = start; i <= total; i += distance) { // i represents each line drawn
			...
			// let line2X = Math.cos(angle_2) * canvas.width,
			// 		line2Y = Math.sin(angle_2) * canvas.width
	
			// let line3X = Math.cos(angle_3) * canvas.width,
			// 		line3Y = Math.sin(angle_3) * canvas.width
	
			// let line4X = Math.cos(angle_4) * canvas.width,
			// 		line4Y = Math.sin(angle_4) * canvas.width
			

			context.beginPath()
			// context.strokeStyle = `hsl(${i * 2}, 100%, 60%)`
			context.strokeStyle = `hsl(${i * hueComplexity}, 100%, 60%)`
			// context.strokeStyle = `hsl(${hue/i*hueComplexity}, 100%, 60%)`
